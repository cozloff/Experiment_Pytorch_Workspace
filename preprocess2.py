...
files = [
    { "filename": "./train/USDJPY_Ticks_29.10.2024-29.10.2024.csv", "symbol": "USD/JPY" },
    { "filename": "./train/AUDUSD_Ticks_29.10.2024-29.10.2024.csv", "symbol": "AUD/USD" },
    { "filename": "./train/EURUSD_Ticks_29.10.2024-29.10.2024.csv", "symbol": "EUR/USD" },
    { "filename": "./train/GBPUSD_Ticks_29.10.2024-29.10.2024.csv", "symbol": "GBP/USD" },
    { "filename": "./train/NZDUSD_Ticks_29.10.2024-29.10.2024.csv", "symbol": "NZD/USD" },
    { "filename": "./train/USDCAD_Ticks_29.10.2024-29.10.2024.csv", "symbol": "USD/CAD" },
    { "filename": "./train/USDCHF_Ticks_29.10.2024-29.10.2024.csv", "symbol": "USD/CHF" },
]
...
def process_file(file_info):
    ...
    # If the dataset exceeds 50,000 rows, downsample to exactly 50,000 rows
    target_size = 50000
    if len(df) > target_size:
        # Generate 50,000 evenly spaced indices over the range of the dataset
        indices = np.linspace(0, len(df) - 1, target_size, dtype=int)
        df = df.iloc[indices]

    # Apply normalization for USD/JPY
    if symbol == "USD/JPY":
        df['Ask'] = (df['Ask'] / 100).round(6)
        df['Bid'] = (df['Bid'] / 100).round(6)
    ...
    # Step 2: Label each tick based on a 0.05% price increase or decrease within the next 5 minutes
    labels = []
    for i in range(len(df)):
        current_time = times[i]
        current_ask = df.iloc[i]['Ask']
        
        # Define a 5-minute window from the current tick
        future_window = df[current_time:current_time + pd.Timedelta(minutes=5)]
        
        # Find the max and min Bid values and their corresponding times within the window
        max_bid = future_window['Bid'].max()
        min_bid = future_window['Bid'].min()
        max_bid_time = future_window['Bid'].idxmax()
        min_bid_time = future_window['Bid'].idxmin()
        
        # Calculate the percentage changes relative to the current ask price
        upward = ((max_bid - current_ask) / current_ask) * 100 >= 0.05
        downward = ((min_bid - current_ask) / current_ask) * 100 <= -0.05
        
        # Determine which threshold is hit first based on the timestamp order
        if upward and downward:
            if max_bid_time < min_bid_time:
                label = 1  # Upward movement occurred first
            else:
                label = 2  # Downward movement occurred first
        elif upward:
            label = 1  # Only upward movement
        elif downward:
            label = 2  # Only downward movement
        else:
            label = 0  # No significant movement
    ...
    text_filename = f"./train2/{symbol.replace('/', '')}_data.txt"
    ...
    # Save the tensor
    tensor_filename = f"./train2/{symbol.replace('/', '')}_data.pt"
