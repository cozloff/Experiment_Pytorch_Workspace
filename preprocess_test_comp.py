...
def process_symbol(symbol):
    ...
    for i in range(len(df)):
        ...
        # Skip if no future window data is available
        if len(future_asks) == 0:
            continue
        ...
        # Check for an upward spike
        upward_spike_found = torch.any(upward_changes >= 0.05)

        # Check for a downward spike
        downward_spike_found = torch.any(downward_changes <= -0.05)

        # Assign labels based on the first spike found
        if upward_spike_found:
            labels[i] = 1
        elif downward_spike_found:
            labels[i] = 2
    ...
    text_filename = f"./test-comp/{symbol.replace('/', '')}_data.txt"
    ...
    tensor_filename = f"./test-comp/{symbol.replace('/', '')}_data.pt"
